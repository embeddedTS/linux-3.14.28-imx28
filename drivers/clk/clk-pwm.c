...
static int clk_pwm_probe(struct platform_device *pdev)
{
	...
	/* XXX: Hack to get around problem with MXS PWM calculation at the
	 * top end of the frequency range
	 */
	ret = pwm_config(pwm, pwm->period , pwm->period);
	if (ret < 0)
		return ret;
	...
	/* XXX: Hack to turn this on instantly */
	clk_pwm_prepare(&(clk_pwm->hw));
